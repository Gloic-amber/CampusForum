				...
				<div class="main-lt" id="div1">
					...
					<div class="box asideProfile">
						...
						<!-- <div class="data-info">
							...
							<dl>
								<dt v-if="UserCreationStatistics.weekRank" class="count">
									{{ UserCreationStatistics.weekRank }}
								</dt>
								<dt v-else>暂无</dt>
								<dd class="font">周排名</dd>
							</dl>
							<dl>
								<dt v-if="UserCreationStatistics.totalRank" class="count">
									{{ UserCreationStatistics.totalRank }}
								</dt>
								<dt v-else>暂无</dt>
	...
	methods: {
		...
		gotoUserHome() {
			...
			var routeUrl = this.$router.resolve({
				name: "UserHome",
				params: { userId: this.profile.authorId },
			});
		...
		async getArticleDetail() {
			...
			await this.$axios.get("/blog/detail?id=" + blogId).then((res) => {
				...
				document.title =
					_this.blogDetail.title + " - " + _this.profile.author + " - 校园博客";
...
.blog-main {
	min-width: 1380px;
	min-height: 100vh;
	background: url("../../assets/img/blogDetail/bgc.jpg");

<template>
	<div class="managecontent">
		...
		<el-tabs v-model="activeName" @tab-click="handleClick">
				...
				<div class="screening-conditions">
					<ul>
						<li
							...
							:key="index"
							@click="ChoseScreen(index)"
							:style="{ color: item.chose ? 'black' : '' }"
						>
							{{ item.title }}
						</li>
					</ul>
						...
						<div class="F-1" v-for="(item, index) in List" :key="index">
							...
							<div class="BlogContent-b">
								...
								>
									...
									<!-- @click="visible2 = true" -->
									<!-- <template>{{ visible2[index] }}</template> -->
									<el-button
			...
			<!-- </el-tab-pane> -->
			<!-- <el-tab-pane label="下载" name="second"> </el-tab-pane>
	...
	methods: {
		async GetData() {
			...
			await this.$axios.get("/blog/console/list", this.config).then((res) => {
				...
				// 将传参的页数标记增大1
				// 创建一个和List数组长度相同的visible2数组，所有元素都初始化为false

				this.visible2.forEach((value, index) => {
					this.$set(this.visible2, index, false);
				});
				// console.log("此时控制博客删除的", this.visible2);
		...
		async infiniteHandler($state) {
			...
			console.log("此时的长度为", this.List.length);
			// console.log("此时所有的博客列表是", this.List);
		},
		//   查找关键字博客列表
		SearchBlog() {
			this.$axios
				.get("/blog/content/search", {
					params: {
						key: this.searchblog,
					},
				})
		...
		ChoseScreen(index) {
			...
			this.config.params.status = index.toString();
			this.config.params.page = 1;
			this.GetData(); //所有博客
		},

		cancelDelete(index) {
			console.log("取消删除的index是： ", index);
			this.$set(this.visible2, index, false);
		},
